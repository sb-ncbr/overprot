...
\begin{itemize}
  \item
    ...
    (\mycite{wwpdb2019}{wwPDB consortium, 2019})
...
\begin{itemize}
  ...
  \item
    ...
    \code{all} (default), a random subset of the domain list is selected.\\
    The family may contain multiple domains from the same PDB entry. If 
    \code{[sample\_selec{\allowbreak}tion]{\allowbreak}unique\_pdb} is \code{True}, then
  ...
  \item
    ...
    \code{{[}download{]}structure\_sources}. The structures are also
...
\item
  ...
  provided in PyMOL module (\mycite{schrodinger}{Schrödinger, LLC.}). In rare cases
  \code{cealign} fails (when the domain is too short) -- in such cases a
...
When an SSE is hovered over or clicked, the viewer dispatches an event (\code{PDB.overprot.hover} or \code{PDB.overprot.select}). 
...
or \code{PDB.overprot.do.select}) by highlighting the selected elements. 
...
the Needleman-Wunsch algorithm (\mycite{needleman1970}{Needleman and Wunsch, 1970})). 

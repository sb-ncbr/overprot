...
if len(args) != 4:
    ...
    exit()
directory, consensus_structure, consensus_sses, out_session = args
use_rainbow = '--no_rainbow' not in options

BASE_COLOR = 'gray80'
DOMAIN = 1 # index of domain name in tuples in samples
MIN_DASH_RADIUS = 0.0  # 0.1  # when radius is dependent on occurrence
MAX_DASH_RADIUS = 1.5
DEFAULT_DASH_RADIUS = 0.25
COLOR_FIELD = 'rainbow' if use_rainbow else 'color'
...
def color_by_annotation(domain, annotation_file, base_color, show_line_segments=True):
    ...
    for sse in sses:
        ...
        color = sse[COLOR_FIELD]
        sel_name = sses_group_name(domain) + '.' + label

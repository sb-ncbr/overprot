...
function autocomplete(inp, arr, showIfEmpty=false) {
    ...
    function inputHandler(){
        ...
        let m = getMatches(val, arr);
        m.forEach(opt => list.appendChild(createListItem(opt, val.length)));
    }
    ...
    function getMatches(currentValue, options, maxNumber=20){
        ...
        for (let i = 0; i < options.length; i++){
            let opt = options[i];
            if (startswith(currentValue, opt.value) && opt.children != undefined) {
                return getMatches(currentValue, opt.children, maxNumber=maxNumber);
            }
            if (startswith(opt.value, currentValue)) {
                result.push(opt);
            }
    ...
    function createListItem(option, nMatchChars){
        ...
        b.setAttribute('keep-open', option.children!=undefined);
        b.setAttribute('do-submit', option.children==undefined);
        /*make the matching letters bold:*/
        let matched = text.substr(0, nMatchChars);
        let completed = text.substr(nMatchChars);
        b.innerHTML = `<span class='matched'>${matched}</span><span class='completed'>${completed}</span><span class='hint'> &nbsp; ${option.hint}</span>`  //&#x22EF;
        /*insert a input field that will hold the current array item's value:*/
        // b.innerHTML += "<input type='hidden' value='" + text + "' close='true'>";
        b.value = text;
        /*execute a function when someone clicks on the item value (DIV element):*/
        b.addEventListener("click", (event) => selectHandler(event, b));
        return b;
    }
    ...
    // inp.addEventListener('click', () => console.log(this.getSelectionRange()));
